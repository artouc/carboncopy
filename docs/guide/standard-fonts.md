...
### 問題：ブラウザとPDFのフォント幅の違い

HTML要素をPDFに変換する際、ブラウザのフォントとPDF標準フォントでは文字幅が異なる場合があります。特にmonospace（等幅）フォントでコードを表示する場合、この差異が顕著になることがあります。

例えば、ブラウザの `Consolas` や `Monaco` と、PDFの `Courier` では1文字あたりの幅が異なるため、変換後にテキストが重なったり、間隔が空きすぎたりすることがあります。

### carboncopyの解決策

carboncopyは、ブラウザで計測したテキスト幅とPDFフォントの理論的な幅を比較し、**文字間隔（Character Spacing）**を自動調整してレイアウトを維持します。

- 文字の形は変形しない（水平スケーリングではなく文字間隔で調整）
- 調整は±2pt以内に制限され、極端な変形を防止
- 調整が不要な場合は元のフォント幅をそのまま使用

### 推奨：HTMLでCourierを指定する

最も正確な変換結果を得るには、HTML側でもPDF標準フォントと同じ `Courier` を使用することを推奨します。
...
::: tip ベストプラクティス
コードスニペットやターミナル出力など、等幅フォントが重要な要素では `font-family: Courier, monospace` を指定することで、ブラウザ表示とPDF出力の両方で一貫した見た目を実現できます。
:::

### 技術詳細

文字間隔の調整は以下の計算式で行われます：

```
文字間隔 = (ブラウザ測定幅 - PDF理論幅) / (文字数 - 1)
```

この調整により、テキスト全体の幅がブラウザで表示された幅と一致します。
